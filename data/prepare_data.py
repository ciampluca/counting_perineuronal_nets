...
DATA_NAMES = ['VGG', 'MBM', 'DCC', 'nuclei', 'BCD', 'HeLa', 'PSU', 'Adipocyte']
...
def create_df_ann_from_imgs(data_path, image_paths, data_name):
    ...
    Useful for VGG, MBM, DCC and Adipocyte
    """
...
def create_df_ann_hela_data(data_path, image_paths):
    """ 
    Create a dataframe containing annotations for the HeLa dataset. 
    """
    
    def load_one_annotation(image_path):
        image_id = image_path.name
        ann_path = data_path / image_id.replace('.pgm', '.mat')
        ann_mat = scipy.io.loadmat(ann_path)
        ann_x, ann_y = ann_mat['gt'][:, 0], ann_mat['gt'][:, 1]
        
        annot = pd.DataFrame({'Y': np.array(ann_y), 'X': np.array(ann_x), 'class': 0})
        image_id = image_id.replace('.pgm', '_cell.pgm')
        annot['imgName'] = image_id
...
def create_df_ann_psu_data(image_paths):
    """ 
    Create a dataframe containing annotations for the PSU dataset. 
    """
    
    def load_one_annotation(image_path):
        image_id = image_path.name
        ann_id = image_id.replace('.bmp', '_detection.mat')
        ann_path = image_path.parent / ann_id
        ann_mat = scipy.io.loadmat(ann_path)
        ann_x, ann_y = ann_mat['detection'][:, 0], ann_mat['detection'][:, 1]
...
def main(args):
    ...
    if args.data_name == 'BCD':    
        ...
        for image_path in image_paths:
            ...
            dst_path.write_bytes(image_path.read_bytes())
    # HeLa
    if args.data_name == 'HeLa':
        image_paths = data_path.glob('*.pgm')
        split = 'train' if data_path.name.startswith('train') else 'test'
        df_ann = create_df_ann_hela_data(data_path, image_paths)
        dst_fld = data_path.parent.parent.parent / split
        Path(dst_fld / 'imgs').mkdir(parents=True, exist_ok=True)
        df_ann.to_csv(dst_fld / 'annotations.csv', encoding='utf-8')
        image_paths = data_path.glob('*.pgm')
        for image_path in image_paths:
            dst_path = dst_fld / 'imgs' / image_path.name.replace('.pgm', '_cell.pgm')
            dst_path.write_bytes(image_path.read_bytes())
    # PSU
    if args.data_name == 'PSU':
        image_paths = data_path.glob('**/*.bmp')
        df_ann = create_df_ann_psu_data(image_paths)
        Path(data_path / 'imgs').mkdir(parents=True, exist_ok=True)
        df_ann.to_csv(data_path / 'annotations.csv', encoding='utf-8')
        image_paths = data_path.glob('**/*.bmp')
        for image_path in image_paths:
            dst_path = data_path / 'imgs' / image_path.name.replace('.bmp', '_cell.bmp')
            dst_path.write_bytes(image_path.read_bytes())
    # Adipocyte
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-name', help="Name of the dataset (can be VGG, MBM, DCC, nuclei, BCD, HeLa, PSU, Adipocyte)")
