...
class PadToSize(object):
    ...
    def __call__(self, image, min_height, min_width, target=None, min_visibility=0.0):
        image_to_tensor_flag = 0
        if isinstance(image, torch.Tensor):
            if image.is_cuda:
                device = image.get_device()
        ...
        transform = A.Compose([
            A.PadIfNeeded(min_height=min_height, min_width=min_width, pad_height_divisor=None, pad_width_divisor=None,
                          value=0),
        ], bbox_params=A.BboxParams(format='pascal_voc', label_fields=['class_labels'], min_visibility=min_visibility))
